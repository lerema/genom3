...
*-C*, *--directory=dir*::
// change to directory dir before reading input files
Change to directory dir before reading the input files. If multiple -C options
are specified, each is interpreted relative to the previous one.
...
+
Each component of 'path' is searched for files matching '\*/template.tcl',
where '*' is interpreted as the template name.
